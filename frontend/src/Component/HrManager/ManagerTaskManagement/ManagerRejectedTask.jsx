  ...
  return (
    <div className="container-fluid py-3">
      <TittleHeader
        ...
        numbers={tasks.filter((task) => task.status === "Rejected" && task.managerEmail.Email===email).length}
      ...
      <div className="row mx-auto">
        {tasks.filter((task) => task.status === "Rejected" && task.managerEmail.Email===email).length > 0 ? (
          tasks
            .filter((task) => task.status === "Rejected" && task.managerEmail.Email ===email)
            .map((task, index) => (
              <div
                key={task._id}
                style={{
